...
var WebMarketVars = {
    ...
    currencySymbol: "%",
    priceRange: [ 5, 40 ], // minimum and maximum range for the price range selector
    priceStep: 1,
    remainingDaysRange: [1, 90],
    remainingDaysStep: 1
};
...
//            }
    ...
    (function() {
        ...
        $container.imagesLoaded(function() {
            ...
            var prepareRemainingDays = function(value) {
                return value === 1? value + " Día": value + " Días";
            };
            
            var $sliderDays = $(".jqueryui-sliderDays-container > div");
            $sliderDays.slider({
                range: true,
                min: WebMarketVars.remainingDaysRange[0],
                max: WebMarketVars.remainingDaysRange[1],
                values: WebMarketVars.remainingDaysRange,
                step: WebMarketVars.remainingDaysStep,
                slide: function(ev, ui) {
                    $(this).parent().siblings(".min-val").val(prepareRemainingDays(ui.values[0]));
                    $(this).parent().siblings(".max-val").val(prepareRemainingDays(ui.values[1]));
                },
                change: function() {
                    updateIsotopeFiltering();
                },
                create: function() {
                    var $sliderParent = $(this).parents(".accordion-body");
                    $sliderParent.find(".min-val").val(prepareRemainingDays($(this).slider("values", 0)));
                    $sliderParent.find(".max-val").val(prepareRemainingDays($(this).slider("values", 1)));
                }
            });
